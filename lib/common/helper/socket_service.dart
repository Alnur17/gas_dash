...
class SocketService {
  ...
  /// Initialize socket and enforce location for drivers
  Future<SocketService> init() async {
    String? token = LocalStorage.getData(key: AppConstant.accessToken);
    String? userId = LocalStorage.getData(key: AppConstant.userId);

    // 🔥 Force location ON if driver
    if (LocalStorage.getData(key: AppConstant.role) == "driver") {
    ...
    _socket.on('onlineUser', (data) {
      ...
      print("👥 Online users: ${onlineUserList.length}");
      print("Users: $onlineUserList");
    });
  ...
  /// 🔥 Enforce user to enable location services & permissions
  Future<void> _checkAndRequestLocationPermission() async {
    bool serviceEnabled = await Geolocator.isLocationServiceEnabled();

    // Keep prompting until location is ON
    while (!serviceEnabled) {
      print('⚠️ Location services are OFF. Opening settings...');
    ...
    while (permission == LocationPermission.denied) {
      print('⚠️ Location permission denied. Requesting again...');
      permission = await Geolocator.requestPermission();
    }
    ...
    print('✅ Location permission granted and GPS enabled.');
  }
  ...
  void _startLocationUpdates() {
    _stopLocationUpdates(); // Prevent duplicate timers
    ...
    print("👤 User Role: ${LocalStorage.getData(key: AppConstant.role)}");

    // If Driver, also send with "getLocation" event
    if (LocalStorage.getData(key: AppConstant.role) == "driver") {
      _locationTimer = Timer.periodic(Duration(seconds: 5), (timer) async {
        if (_socket.connected) {
          try {
            ...
            _socket.emit('getLocation', locationData);
            print('🚚 Driver location sent: $locationData');
          } catch (e) {
  ...
  void _handleIncomingFriends(dynamic data) {
    if (data is Map<String, dynamic> && data.containsKey('receiver')) {
      _friendList.add(data);
      print('👥 Friend added: ${_friendList.length}');
    }
  }
}
